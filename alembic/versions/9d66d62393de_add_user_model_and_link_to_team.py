...
def upgrade() -> None:
    ...
    if not users_table_exists:
        ...
        # Table đã tồn tại, chỉ cần đảm bảo indexes tồn tại
        conn.execute(sa.text("""
    ...
    if not teams_table_exists:
        ...
        # Table đã tồn tại, chỉ cần đảm bảo indexes và status column tồn tại
        conn.execute(sa.text("""
            CREATE INDEX IF NOT EXISTS ix_teams_id ON teams(id);
            CREATE UNIQUE INDEX IF NOT EXISTS ix_teams_order_id ON teams(order_id);
            ...
            DO $$ BEGIN
                ALTER TABLE teams ADD COLUMN status team_status NOT NULL DEFAULT 'REGISTERED';
            EXCEPTION
